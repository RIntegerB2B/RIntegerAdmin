...
@Component({
  selector: 'app-subscribe-customer',
...
export class SubscribeCustomerComponent implements OnInit {
  ...
  arrayBuffer: any;
  file: File;
  subscribeDetailsForm: FormGroup;
  newSubscribe: Customer[] = [];
  ...
  createForm() {
    this.subscribeDetailsForm = this.fb.group({
      _id: [],
  ...
  updateSubscribe(subscribeDetailsForm: FormGroup, row) {
 ...
 */
  ...
  getAllSubscribe() {
    this.customerService.getAllSubscribeCustomer().subscribe(data => {
      this.newSubscribe = data;
      /* this.temp = [...data]; */
      this.temp = data;
      console.log(this.newSubscribe);
    }, error => {
      console.log(error);
    });
  }
